...
exports.createProfile = async (req, res) => {
  try {
    const { userId, username, email, firstName, lastName, phone, address, dateOfBirth, initialBalance } = req.body;
    ...
    // Set initial balance (default 1000.00 for demo purposes)
    const balance = initialBalance !== undefined ? initialBalance : 1000.00;

    const [result] = await pool.query(
      `INSERT INTO user_profiles (user_id, username, email, first_name, last_name, phone, address, date_of_birth, balance) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [userId, username || null, email || null, firstName || null, lastName || null, phone || null, address || null, dateOfBirth || null, balance]
    );
...
// POST: Create user profile with JWT authentication (for users who registered before profile creation was added)
exports.createProfileWithAuth = async (req, res) => {
  try {
    ...
    // Get username and email from request user (from JWT token)
    ...
    const [result] = await pool.query(
      `INSERT INTO user_profiles (user_id, username, email, first_name, last_name, phone, address, date_of_birth) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [userId, username, email, firstName || null, lastName || null, phone || null, address || null, dateOfBirth || null]
    );
...
exports.getBalance = async (req, res) => {
  try {
    ...
    res.json({
      ...
      balance: parseFloat(profiles[0].balance) || 0
    });
  } catch (error) {
...
// PUT: Update user balance (deposit/withdraw)
exports.updateBalance = async (req, res) => {
  try {
    ...
    const currentBalance = parseFloat(profiles[0].balance) || 0;
    let newBalance;
