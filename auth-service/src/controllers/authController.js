...
exports.login = async (req, res) => {
  try {
    ...
    // Create JWT token with required payload: sub, name, iat, exp
    // iat and exp are automatically added by jwt.sign when expiresIn is set
    const token = jwt.sign(
      {
        sub: user.id.toString(),  // Subject (user ID)
        name: user.username,       // User's name
        email: user.email,         // Additional info
